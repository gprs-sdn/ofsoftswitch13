...
void crc_init_table24() {
	...
	for (i = 0; i < sizeof(poly_bits24); i++)
		poly24|= (uint32_t)(1) << (23-poly_bits24[i]);

	for (i = 0; i < 256; i++) {
		uint32_t part24 = (uint32_t)(i);
			
		for (j = 0; j < 8; j++)
			part24 = (part24 & 1) ? (part24>>1)^poly24 : (part24>>1);

		crc_table24[i] = part24 & 0x00ffffff;
	}
}

uint32_t crc_compute24(uint8_t* buffer, uint32_t length, uint32_t initstate) {
	register uint32_t crc = INVERT24(initstate);

	if (!crc_table24[1])
		crc_init_table24();

	while (length--) 
		crc = crc_table24[(uint8_t)(crc)^(*buffer++)] ^ (crc>>8);
